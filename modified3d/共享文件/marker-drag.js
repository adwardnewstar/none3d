...
function setupDraggableMarker(app) {
    ...
    function getGroupPosition() {
        return markerObjects.x ? markerObjects.x.position.clone() : new v3d.Vector3();
    }
    ...
    function hitTestAxis(ndc) {
        ...
        if (intersects.length > 0) {
            ...
            if (hitObj.name === '标记X') return { axis: 'x', point: intersects[0].point };
            if (hitObj.name === '标记Y') return { axis: 'y', point: intersects[0].point };
            if (hitObj.name === '标记Z') return { axis: 'z', point: intersects[0].point };
        }
        return null;
    }
    ...
    function onPointerMoveCapture(e) {
        ...
        if (activeAxis === 'x') {
            newPos.x = groupStartPos.x + t * sign;
        } else if (activeAxis === 'y') {
            newPos.z = groupStartPos.z + t * sign;
        } else if (activeAxis === 'z') {
            newPos.y = groupStartPos.y + t * sign;
        }

        // 应用视锥边界约束
        if (viewBounds) {
            newPos.y = Math.max(viewBounds.yMin, Math.min(newPos.y, viewBounds.yMax));
            newPos.x = Math.max(-viewBounds.xMax, Math.min(newPos.x, viewBounds.xMax));
            newPos.z = Math.max(-viewBounds.zMax, Math.min(newPos.z, viewBounds.zMax));
        }
    ...
    function onPointerUpCapture(e) {
        ...
        if (capturedPointerId !== null) {
            try { domElement.releasePointerCapture(capturedPointerId); } catch(_) {}
        ...
        localStorage.setItem(STORAGE_KEY, JSON.stringify({ x: pos.x, y: pos.y, z: pos.z }));
    }
    ...
    // ---- 公开方法 ----
    return {
        getMarker: function() { return getGroupPosition(); },
